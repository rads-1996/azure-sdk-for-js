...
```bash
pnpm add -g @azure-tools/typespec-client-generator-cli
```
...
{
  "scripts": {
    "generate:client": "tsp-client update -d && pnpm run format && dev-tool customization apply --skip index.ts",
    "build": "pnpm run clean && dev-tool run build-package && dev-tool run extract-api",
